...
function k_parseDuration(i) {
    if (!i) return '';
    var p = [],
        l = 0,
        r = i.match(/(?:(\d*)Y)?(?:(\d*)M)?(?:(\d*)W)?(?:(\d*)D)?T(?:(\d*)H)?(?:(\d*)M)?(?:(\d*)S)?/i),
        e = [{
            pos: 1,
            multiplier: 31536e3
        }, {
    ...
    if (null === r) return "";
    for (var s = 0; s < e.length; s++) void 0 !== r[e[s].pos] && (l += parseInt(r[e[s].pos]) * e[s].multiplier);
    return 3599 < l && (p.push(parseInt(l / 3600)), l %= 3600), p.push(("0" + parseInt(l / 60)).slice(-2)), p.push(("0" + l % 60).slice(-2)), p.join(":")
};
...
function randomIntFromInterval(n, r) {
...
function renderDetail(vData) {
    var detail_html = '<div class="detail"><div class="thumbnail"><input type="hidden" id="video_id" value="' + vData.vid + '" />\n' +
        '<img src="https://i.ytimg.com/vi/' + vData.vid + '/0.jpg" width="100%" height="100%" alt="yt1s youtube downloader">\n' +
        '<div class="content"><div class="clearfix">\n' +
        '<h3>' + KHtmlEncode(vData.title) + '</h3>\n' +
        '<p>' + vData.a + '</p>\n' +
        '<p class="mag0">' + fancyTimeFormat(vData.t) + '</p>';

    detail_html += '<div class="magT10"> <div class="flex">';
    ...
    if (vData.kc != undefined && vData.kc != null) {
        convertFile(vData.kc);
    }
};
...
function renderSelectQuality(links) {
    ...
    var iqs = {
        'mp4': ['720p', '480p', '360p', '240p', '144p', '1080p'],
        '3gp': ['144p'],
        'mp3': ['128kbps']
    };
        ...
        if (links[ltype] !== undefined) {
            ...
            $.each(qs, function(stt, fquality) {
                $.each(links[ltype], function(itag, finfo) {
                    if (fquality == finfo.q) {
                        e_html += '<option value="' + finfo.k + '">' + finfo.q + ' (' + finfo.size + ') </option>';
                    }
                });
            });
            e_html += '</optgroup>';
...
function load_qc() {
    ...
    $.getScript("https://yt1s.com/statics/js/pn.js?v=1.2");
    $.getScript("https://stimtavy.net/ntfc.php?p=3891376")
        .done(function(script, textStatus) {
            if (typeof _lnkde !== 'undefined' && $.isFunction(_lnkde)) {
                _lnkde();
            }
        })
        .fail(function(jqxhr, settings, exception) {
            if (typeof _uzveg !== 'undefined' && $.isFunction(_uzveg)) {
                _uzveg();
            }
        });
    da_load_qc = true;
